# -*- coding: utf-8 -*-
"""
Created on Wed Jun 18 18:04:31 2025
...
def draw(anal_data):
    fig, axes = prototype_perf_from_dysttg_dur.draw(anal_data, param = 'lower', comp_sttg = 'gmin')
    for idx in range(3):
        ax = axes[0][idx]
        plt.sca(ax)
        if idx == 0: lower_ylim, upper_ylim, ygap, y_coef = 0, 50, 25, 100
        elif idx == 1: lower_ylim, upper_ylim, ygap, y_coef = -6, 3, 3, 100
        elif idx == 2: lower_ylim, upper_ylim, ygap, y_coef = 0, 10, 5, 1000
        else: pass
        # if idx == 0: lower_ylim, upper_ylim, ygap = 0, 75, 25
        # elif idx == 1: lower_ylim, upper_ylim, ygap = -15, 60, 15
