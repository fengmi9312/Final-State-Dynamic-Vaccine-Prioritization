...
def draw(anal_data):
    ...
    center_x_list = [17.5, 38.5, 46.5, 78.5]
    ...
    ax.add_patch(patches.Rectangle((110.5 - width / 2, center_y - height / 2), 30.4, height, edgecolor='black', facecolor='none', linestyle = ':', linewidth = 1.5, alpha = 0.6, clip_on = False, transform = blended_transform_factory(ax.transData,ax.transAxes)))
    ax.text(125, 1.08, 'Disorder', fontsize = 10, transform = blended_transform_factory(ax.transData,ax.transAxes), ha = 'center', va = 'bottom')
    ...
    plt.yticks(np.arange(0, 21, 5) / 10, np.arange(0, 21, 5) / 10)
    plt.xticks(np.arange(0, 141, 70), np.arange(0, 141, 70))
    values = anal_data['allocs_from_time'][f'effr_min_{target}_{target}_{file_idx}'].values
    for center_x in center_x_list:
        ax.annotate('', xy=(center_x, (values[1][int(center_x)] + values[1][int(center_x) + 1]) / 2), xytext=(center_x, 2.23), ha='center', va='top', arrowprops=dict(facecolor='black', arrowstyle='->', shrinkB=7, linestyle = '--', alpha = 0.6), fontsize = 8)
