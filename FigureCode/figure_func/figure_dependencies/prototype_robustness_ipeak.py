...
def draw(anal_data, **kwargs):
    ####################################################
    robust_type = kwargs.pop('robust_type', 'r0')
    ####################################################
    scale_prop = 12
    ...
    for i in range(2):
        ...
        for sttg in sttg_formats.keys():
            res = anal_data['ipeak_from_dysttg'][f'example_42_{sttg}']['res'] if robust_type == 'time' else anal_data['ipeak_from_dysttg'][sttg]['res']
            x = np.arange(len(res)) * 0.01 if robust_type == 'time' else 1.1 + np.arange(120) * 0.1 / 3
            plt.plot(x, res, color = sttg_formats[sttg][0], linestyle = sttg_formats[sttg][1], label = sttg_formats[sttg][2], linewidth = 2)
        ...
        if robust_type == 'time': figure_setting.remove_spines(ax, ['top', 'right'])
