...
def draw(anal_data, **kwargs):
    ...
    grid_attrs = [[{'pos': (0, 0), 'size': (14, 5)}, {'pos': (22, 0), 'size': (14, 5)},
                   ...
                   {'pos': (0, 27), 'size': (36, 6)}, {'pos': (0, 36), 'size': (36, 6)}],
                  
                  [{'pos': (50, 0), 'size': (14, 5)}, {'pos': (72, 0), 'size': (14, 5)},
                   {'pos': (50, 6), 'size': (14, 14)}, {'pos': (72, 6), 'size': (14, 14)}, 
                   {'pos': (50, 27), 'size': (36, 6)}, {'pos': (50, 36), 'size': (36, 6)}],
                 
                 [{'pos': (100, 0), 'size': (14, 5)}, {'pos': (122, 0), 'size': (14, 5)},
                  {'pos': (100, 6), 'size': (14, 14)}, {'pos': (122, 6), 'size': (14, 14)}, 
                  {'pos': (100, 27), 'size': (36, 6)}, {'pos': (100, 36), 'size': (36, 6)}],
                 [{'pos': (138, 6), 'size': (1, 14)}], [{'pos': (138, 27), 'size': (1, 15)}]]
    ...
    for target_idx, target in enumerate(['c', 'd', 'y']):
        for r0_idx in range(2):
            ax = axes[target_idx][2 + r0_idx]
            plt.sca(ax)
            mat_data = anal_data['impact_to_dysttg'][f'{sttg}_{target}_{r0_idx}'].values
            mat_max = np.max(mat_data)
            mat_res = anal_data['impact_to_dysttg'][f'{sttg}_{target}_{r0_idx}'].values / mat_max
            ...
            if r0_idx == 0: ax.text(18 / 14, 1.6, target_names[target_idx], fontsize = 12, transform = ax.transAxes, ha = 'center', va = 'bottom')
            ax.text(0.5, 1.1, f'$R_0 = {r0_values[r0_idx]}$ (Direct Effects)', fontsize = 10, transform = ax.transAxes, ha = 'center', va = 'bottom')
            
            ax = axes[target_idx][4 + r0_idx]
            plt.sca(ax)
            res = anal_data['allocs_dysttg_from_time'][f'alloc_{sttg}_{target}_{r0_idx * 33 + 9}']
            ax, im = pcolor_heatmap(ax, res)
            if r0_idx == 1: 
                figure_setting.set_xylabel(ax, 'Time (d)', '', fontsize = label_fontsize, xlabel_coords = -0.24, ylabel_coords = -0.08)
                ax.text(-0.1, 11 / 8, 'Age Group', fontsize = label_fontsize, transform = ax.transAxes, ha = 'right', va = 'center', rotation = 90)
