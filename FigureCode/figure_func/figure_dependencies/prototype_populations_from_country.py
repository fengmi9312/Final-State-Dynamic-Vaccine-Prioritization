# -*- coding: utf-8 -*-
"""
Created on Sun Jul  6 15:41:03 2025
...
def draw(anal_data, **kwargs):
    ...
    for idx, country in enumerate(sorted_countries):
        ax = axes[0][idx]
        plt.sca(ax)
        plt.bar(np.arange(len(age_groups)), country_data[country]['populations'], color = 'tab:green', edgecolor = 'black', linewidth = 1.5)
        ...
        figure_setting.set_xylabel(ax, 'Age Group', 'Populations (%)', label_fontsize, xlabel_coords = -0.2, ylabel_coords = -0.12)
        ...
        plt.yticks(np.arange(0, 25, 6) / 100, np.arange(0, 25, 6))
    return fig, axes