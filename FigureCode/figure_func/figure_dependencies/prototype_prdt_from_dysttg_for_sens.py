...
def draw(anal_data, **kwargs):
    ####################################################
    param = kwargs.pop('param', 'v14_d14')
    ...
    # adata = anal_data['prdt_from_dysttg_for_sens'][f'{param}_r0']
    xlabel = r'$R_0$'
    mark_text = {'c': 'Cumulative Infections', 'd': 'Deaths', 'y': 'YLL'}
    # vlines = [1.5, 2.1, 3.5]
    r0_arr = anal_data['prdt_from_dysttg_for_sens'][f'{param}_r0']['r0'] if r0_limit is None else anal_data['prdt_from_dysttg_for_sens'][f'{param}_r0']['r0'][:r0_limit]
    ...
    for idx, target in enumerate(['c', 'd', 'y']):
        ...
        for sttg_idx, sttg_key in enumerate(sttgs):
            ...
            sttg_res = anal_data['prdt_from_dysttg_for_sens'][f'{param}_{sttg}_{target}']
        ...
        if target == 'y' and r0_limit is None:
            sttg_min_res = anal_data['prdt_from_dysttg_for_sens'][f'{param}_min_{target}_{target}']
            sttg_gmin_res = anal_data['prdt_from_dysttg_for_sens'][f'{param}_gmin_{target}_{target}']
