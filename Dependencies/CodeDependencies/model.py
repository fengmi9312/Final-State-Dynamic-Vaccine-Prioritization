...
class sir_delta:
    ...
    def __init__(self, i0, populations, contacts, k,
        ...
        if haz_cum is None:
            ...
            self.__cum_len = np.where(haz_cum <= 0)[0][0] if np.any(haz_cum <= 0) else len(haz_cum)
            self.__actl_rem_len = np.where(srv_rem <= 0)[0][0] if np.any(srv_rem <= 0) else len(srv_rem)
            self.__cum_len = min(self.__cum_len, self.__actl_rem_len)
            self.__srv_rem = deepcopy(srv_rem[:self.__cum_len])
            self.__haz_cum = deepcopy(haz_cum[:self.__cum_len])
            self.__rem_m = 1 / (np.diff(1 - srv_rem) * np.arange(len(srv_rem) - 1) * step).sum()
            self.__inf_m = np.log(1 / (1 - haz_cum).prod()) * self.__rem_m
...
#############################################################################
    def calc_lams(self, delay_prdt = True, prdt_type = 'non_mar'):
        if prdt_type == 'mar': return {'lambda_eff': self.__inf_m / self.__rem_m, 'memory_eff': 0}
        elif prdt_type == 'non_mar':
    ...
    def calc_trans_grad(self, prdt_type = 'non_mar'):
        if self.__time_int < self.__delay:
            return None
        
        if prdt_type == 'non_mar': return np.diag((- self.__get_eff() * self.__eta) * self.__j[self.__time_int])
        elif prdt_type == 'mar': return np.diag((- self.__get_eff() * self.__eta) * func.mulf(self.__totmat, self.__inf_m * self.__i[self.__time_int]))
        else: return None
    
    def calc_trans_grad_target(self, target = 'c', prdt_type = 'non_mar'):
        if target == 'c':
            return self.calc_trans_grad(prdt_type = prdt_type) @ self.__populations
        elif target == 'd':
            return self.calc_trans_grad(prdt_type = prdt_type) @ (self.__populations * self.__ifrs)
        elif target == 'y':
            return self.calc_trans_grad(prdt_type = prdt_type) @ (self.__populations * self.__ifrs * self.__ylls)
        else:
            return None
    
    def calc_steady_grad(self, vac_alloc, prdt_type = 'non_mar', gtol = 1e-6):
        _vac_alloc = np.minimum(vac_alloc, self.__s[self.__current_time_int]) * self.__eta * self.__get_eff()
        lams = self.calc_lams(prdt_type = prdt_type)
        pos = self.__time_int
        rt= self.__r[pos]
        ...
        c_arr = self.calc_vac_prdt(vac_alloc, prdt_type = prdt_type)
    ...
    def calc_reduction_eff_target(self, target = 'c', vac_avail = None):
        if vac_avail is None or vac_avail == 0:
            ...
            grad = self.calc_steady_grad_target(target = target, vac_alloc = zero_alloc, prdt_type = 'non_mar')
    ...
    def optimize_continual_vac_alloc(self, vac_avail, target = 'c', prdt_type = 'non_mar', target_type = 'steady'):
        zero_alloc = np.zeros(self.__group_amount)
        if prdt_type == 'non_mar' and target_type == 'steady': grad = self.calc_steady_grad_target(target = target, vac_alloc = zero_alloc, prdt_type = prdt_type)
        elif prdt_type == 'mar' and target_type == 'steady': grad = self.calc_steady_grad_target(target = target, vac_alloc = zero_alloc, prdt_type = prdt_type)
        elif prdt_type == 'non_mar' and target_type == 'trans': grad = self.calc_trans_grad_target(target = target, vac_alloc = zero_alloc, prdt_type = prdt_type)
        elif prdt_type == 'non_mar' and target_type == 'trans': grad = self.calc_trans_grad_target(target = target, vac_alloc = zero_alloc, prdt_type = prdt_type)
        else: grad = None
    ...
    def calc_prdt(self, prdt_type = 'non_mar', delay_prdt = True, ctol = 1e-10, init_c = None):
        lams = self.calc_lams(delay_prdt = delay_prdt, prdt_type = prdt_type)
        pos = self.__time_int if delay_prdt else self.__current_time_int
        rt= self.__r[pos]
    ...
    def calc_vac_prdt(self, vac_alloc, prdt_type = 'non_mar', ctol = 1e-10, init_c = None):
        _vac_alloc = np.minimum(vac_alloc, self.__s[self.__current_time_int]) * self.__eta * self.__get_eff()
        lams = self.calc_lams(prdt_type = prdt_type)
        pos = self.__time_int
        c_tmp = self.__c[pos] if init_c is None else init_c
