...
class epidemic_model:
    ...
    def __init__(self, i0, populations, contacts, k,
                 asym_prob, exp_srv_rem,
                 asym_srv_inf, asym_srv_rem,
                 presym_srv_inf, presym_srv_rem,
        ...
        if asym_srv_inf[0] != 1 or asym_srv_rem[0] != 1: print('srv error!')
        self.__asym_actl_inf_len = np.where(asym_srv_inf <= 0)[0][0] if np.any(asym_srv_inf <= 0) else len(asym_srv_inf)
        self.__asym_actl_rem_len = np.where(asym_srv_rem <= 0)[0][0] if np.any(asym_srv_rem <= 0) else len(asym_srv_rem)
        self.__asym_cum_len = min(self.__asym_actl_inf_len - 1, self.__asym_actl_rem_len)
        self.__asym_haz_inf = 1 - asym_srv_inf[1:self.__asym_actl_inf_len] / asym_srv_inf[:self.__asym_actl_inf_len - 1]
        self.__asym_srv_rem = deepcopy(asym_srv_rem[:self.__asym_actl_rem_len])
        self.__asym_haz_cum = self.__asym_haz_inf[:self.__asym_cum_len] * self.__asym_srv_rem[:self.__asym_cum_len]
        self.__asym_dist_rem = - np.diff(self.__asym_srv_rem)
        
        if presym_srv_inf[0] != 1 or presym_srv_rem[0] != 1: print('srv error!')
        self.__presym_actl_inf_len = np.where(presym_srv_inf <= 0)[0][0] if np.any(presym_srv_inf <= 0) else len(presym_srv_inf)
        self.__presym_actl_rem_len = np.where(presym_srv_rem <= 0)[0][0] if np.any(presym_srv_rem <= 0) else len(presym_srv_rem)
        self.__presym_cum_len = min(self.__presym_actl_inf_len - 1, self.__presym_actl_rem_len)
        self.__presym_haz_inf = 1 - presym_srv_inf[1:self.__presym_actl_inf_len] / presym_srv_inf[:self.__presym_actl_inf_len - 1]
        self.__presym_srv_rem = deepcopy(presym_srv_rem[:self.__presym_actl_rem_len])
        self.__presym_haz_cum = self.__presym_haz_inf[:self.__presym_cum_len] * self.__presym_srv_rem[:self.__presym_cum_len]
        self.__presym_dist_rem = - np.diff(self.__presym_srv_rem)
    ...
    def return_to_init(self):
        ...
        self.__asym_in = np.zeros((self.__period_len, self.__group_amount))
        self.__asym = np.zeros((self.__period_len, self.__group_amount))
        self.__presym_in = np.zeros((self.__period_len, self.__group_amount))
        self.__presym = np.zeros((self.__period_len, self.__group_amount))
        self.__sym_in = np.zeros((self.__period_len, self.__group_amount))
        self.__sym = np.zeros((self.__period_len, self.__group_amount))
        self.__a_r = np.zeros((self.__period_len, self.__group_amount))
        self.__s_r = np.zeros((self.__period_len, self.__group_amount))
        ...
        self.__asym_in_tot = np.zeros(self.__period_len)
        self.__asym_tot = np.zeros(self.__period_len)
        self.__presym_in_tot = np.zeros(self.__period_len)
        self.__presym_tot = np.zeros(self.__period_len)
        self.__sym_in_tot = np.zeros(self.__period_len)
        self.__sym_tot = np.zeros(self.__period_len)
        self.__a_r_tot = np.zeros(self.__period_len)
        self.__s_r_tot = np.zeros(self.__period_len)
        ...
        self.__asym_in[0] = np.zeros(self.__group_amount)
        self.__asym[0] = np.zeros(self.__group_amount)
        self.__presym_in[0] = np.zeros(self.__group_amount)
        self.__presym[0] = np.zeros(self.__group_amount)
        self.__sym_in[0] = np.zeros(self.__group_amount)
        self.__sym[0] = np.zeros(self.__group_amount)
        self.__a_r[0] = np.zeros(self.__group_amount)
        self.__s_r[0] = np.zeros(self.__group_amount)
        self.__r[0] = np.zeros(self.__group_amount)
        self.__c[0] = self.__i0
        self.__d[0] = self.__s_r[0] * self.__ifrs
        self.__y[0] = self.__s_r[0] * self.__ifrs * self.__ylls
    ...
    def __expand_len(self):
        ...
        self.__asym_in = np.append(self.__asym_in, np.zeros((self.__period_len, self.__group_amount)), axis = 0)
        self.__asym = np.append(self.__asym, np.zeros((self.__period_len, self.__group_amount)), axis = 0)
        self.__presym_in = np.append(self.__presym_in, np.zeros((self.__period_len, self.__group_amount)), axis = 0)
        self.__presym = np.append(self.__presym, np.zeros((self.__period_len, self.__group_amount)), axis = 0)
        ...
        self.__a_r = np.append(self.__a_r, np.zeros((self.__period_len, self.__group_amount)), axis = 0)
        self.__s_r = np.append(self.__s_r, np.zeros((self.__period_len, self.__group_amount)), axis = 0)
        ...
        self.__asym_in_tot = np.append(self.__asym_in_tot, np.zeros(self.__period_len))
        self.__asym_tot = np.append(self.__asym_tot, np.zeros(self.__period_len))
        self.__presym_in_tot = np.append(self.__presym_in_tot, np.zeros(self.__period_len))
        self.__presym_tot = np.append(self.__presym_tot, np.zeros(self.__period_len))
        ...
        self.__a_r_tot = np.append(self.__a_r_tot, np.zeros(self.__period_len))
        self.__s_r_tot = np.append(self.__s_r_tot, np.zeros(self.__period_len))
...
###############################################################################            
    ...
    def __calc_inf(self):
        return func.mulf(self.__totmat, 
                         self.__asym_haz_cum[:func.get_tail(self.__time_int, self.__asym_cum_len)] @ self.__asym_in[self.__time_int:func.get_head(self.__time_int, self.__asym_cum_len):-1] \
                         + self.__presym_haz_cum[:func.get_tail(self.__time_int, self.__presym_cum_len)] @ self.__presym_in[self.__time_int:func.get_head(self.__time_int, self.__presym_cum_len):-1] \
    ...
    def __calc_tot(self):
        ...
        self.__asym_in_tot[self.__time_int] = self.__asym_in[self.__time_int] @ self.__populations
        self.__presym_in_tot[self.__time_int] = self.__presym_in[self.__time_int] @ self.__populations
        ...
        self.__asym_tot[self.__time_int] = self.__asym[self.__time_int] @ self.__populations
        self.__presym_tot[self.__time_int] = self.__presym[self.__time_int] @ self.__populations
        self.__sym_tot[self.__time_int] = self.__sym[self.__time_int] @ self.__populations
        self.__a_r_tot[self.__time_int] = self.__a_r[self.__time_int] @ self.__populations
        self.__s_r_tot[self.__time_int] = self.__s_r[self.__time_int] @ self.__populations
    ...
    def spread_once(self):
        ...
        self.__asym_in[self.__time_int + 1] = self.__asym_prob * self.__exp_dist_rem[:func.get_tail(self.__time_int,self.__exp_actl_rem_len-1)] @ self.__exp_in[self.__time_int:func.get_head(self.__time_int,self.__exp_actl_rem_len-1):-1]
        self.__presym_in[self.__time_int + 1] = (1 - self.__asym_prob) * self.__exp_dist_rem[:func.get_tail(self.__time_int,self.__exp_actl_rem_len-1)] @ self.__exp_in[self.__time_int:func.get_head(self.__time_int,self.__exp_actl_rem_len-1):-1]
        self.__sym_in[self.__time_int + 1] = self.__presym_dist_rem[:func.get_tail(self.__time_int,self.__presym_actl_rem_len-1)] @ self.__presym_in[self.__time_int:func.get_head(self.__time_int,self.__presym_actl_rem_len-1):-1]
        self.__a_r[self.__time_int + 1] = self.__a_r[self.__time_int] + self.__asym_dist_rem[:func.get_tail(self.__time_int,self.__asym_actl_rem_len-1)] @ self.__asym_in[self.__time_int:func.get_head(self.__time_int,self.__asym_actl_rem_len-1):-1]
        self.__s_r[self.__time_int + 1] = self.__s_r[self.__time_int] + self.__sym_dist_rem[:func.get_tail(self.__time_int,self.__sym_actl_rem_len-1)] @ self.__sym_in[self.__time_int:func.get_head(self.__time_int,self.__sym_actl_rem_len-1):-1]
        ...
        self.__asym[self.__time_int + 1] = self.__asym_srv_rem[:func.get_tail(self.__time_int + 1,self.__asym_actl_rem_len)] @ self.__asym_in[self.__time_int + 1:func.get_head(self.__time_int + 1,self.__asym_actl_rem_len):-1]
        self.__presym[self.__time_int + 1] = self.__presym_srv_rem[:func.get_tail(self.__time_int + 1,self.__presym_actl_rem_len)] @ self.__presym_in[self.__time_int + 1:func.get_head(self.__time_int + 1,self.__presym_actl_rem_len):-1]
        ...
        self.__r[self.__time_int + 1] = self.__a_r[self.__time_int + 1] + self.__s_r[self.__time_int + 1]
        self.__d[self.__time_int + 1] = self.__s_r[self.__time_int + 1] * self.__ifrs
        self.__y[self.__time_int + 1] = self.__s_r[self.__time_int + 1] * self.__ifrs * self.__ylls
...
###############################################################################
    ...
    def get_asym_in(self):
        return self.__asym_in[:self.__current_time_int + 1]
    
    def get_presym_in(self):
        return self.__presym_in[:self.__current_time_int + 1]
    ...
    def get_asym(self):
        return self.__asym[:self.__current_time_int + 1]
    
    def get_presym(self):
        return self.__presym[:self.__current_time_int + 1]
    
    def get_sym(self):
        return self.__sym[:self.__current_time_int + 1]
    
    def get_a_r(self):
        return self.__a_r[:self.__current_time_int + 1]
    
    def get_s_r(self):
        return self.__s_r[:self.__current_time_int + 1]
    ...
    def get_asym_in_tot(self):
        return self.__asym_in_tot[:self.__current_time_int + 1]
    
    def get_presym_in_tot(self):
        return self.__presym_in_tot[:self.__current_time_int + 1]
    ...
    def get_asym_tot(self):
        return self.__asym_tot[:self.__current_time_int + 1]
    
    def get_presym_tot(self):
        return self.__presym_tot[:self.__current_time_int + 1]
    
    def get_sym_tot(self):
        return self.__sym_tot[:self.__current_time_int + 1]
    
    def get_a_r_tot(self):
        return self.__a_r_tot[:self.__current_time_int + 1]
    
    def get_s_r_tot(self):
        return self.__s_r_tot[:self.__current_time_int + 1]
    ...
    def get_x_tot(self, target):
        if target == 'exp_in':
            ...
            return self.get_asym_in_tot()
        elif target == 'presym_in':
            return self.get_presym_in_tot()
        elif target == 'sym_in':
            ...
            return self.get_asym_tot()
        elif target == 'presym':
            return self.get_presym_tot()
        elif target == 'sym':
            return self.get_sym_tot()
        elif target == 'a_r':
            return self.get_a_r_tot()
        elif target == 's_r':
            return self.get_s_r_tot()
        elif target == 'r':
    ...
    def getc_x(self, target):
        if target == 'exp_in':
            ...
            return self.getc_asym_in()
        elif target == 'presym_in':
            return self.getc_presym_in()
        elif target == 'sym_in':
            ...
            return self.getc_asym()
        elif target == 'presym':
            return self.getc_presym()
        elif target == 'sym':
            return self.getc_sym()
        elif target == 'a_r':
            return self.getc_a_r()
        elif target == 's_r':
            return self.getc_s_r()
        elif target == 'r':
            return self.getc_r()
        elif target == 'c':
            return self.getc_c()
    ...
    def __getx_x(self, target, pos):
        if target == 'exp_in':
            ...
            return self.__asym_in[pos]
        elif target == 'presym_in':
            return self.__presym_in[pos]
        elif target == 'sym_in':
            ...
            return self.__asym[pos]
        elif target == 'presym':
            return self.__presym[pos]
        elif target == 'sym':
            return self.__sym[pos]
        elif target == 'a_r':
            return self.__a_r[pos]
        elif target == 's_r':
            return self.__s_r[pos]
        elif target == 'r':
            return self.__r[pos]
        elif target == 'c':
            return self.__c[pos]
    ...
    def getc_asym_in(self):
        return self.__asym_in[self.__current_time_int]
    
    def getc_presym_in(self):
        return self.__presym_in[self.__current_time_int]
    ...
    def getc_asym(self):
        return self.__asym[self.__current_time_int]
    
    def getc_presym(self):
        return self.__presym[self.__current_time_int]
    
    def getc_sym(self):
        return self.__sym[self.__current_time_int]
    
    def getc_a_r(self):
        return self.__a_r[self.__current_time_int]
    
    def getc_s_r(self):
        return self.__s_r[self.__current_time_int]
    ...
    def getc_asym_in_tot(self):
        return self.__asym_in_tot[self.__current_time_int]
    
    def getc_presym_in_tot(self):
        return self.__presym_in_tot[self.__current_time_int]
    ...
    def getc_asym_tot(self):
        return self.__asym_tot[self.__current_time_int]
    
    def getc_presym_tot(self):
        return self.__presym_tot[self.__current_time_int]
    
    def getc_sym_tot(self):
        return self.__sym_tot[self.__current_time_int]
    
    def getc_a_r_tot(self):
        return self.__a_r_tot[self.__current_time_int]
    
    def getc_s_r_tot(self):
        return self.__s_r_tot[self.__current_time_int]
    ...
    def getc_x_tot(self, target):
        if target == 'exp_in':
            ...
            return self.getc_asym_in_tot()
        elif target == 'presym_in':
            return self.getc_presym_in_tot()
        elif target == 'sym_in':
            ...
            return self.getc_asym_tot()
        elif target == 'presym':
            return self.getc_presym_tot()
        elif target == 'sym':
            return self.getc_sym_tot()
        elif target == 'a_r':
            return self.getc_a_r_tot()
        elif target == 's_r':
            return self.getc_s_r_tot()
        elif target == 'r':
