...
def analyze(edata):
    ...
    for expr_param in expr_params:
        ...
        #         last_idx = np.inf
        #         for sttg in ['under_20', '20-49', '20+', '60+', 'all_ages', 'min', 'gmin', 'mmin', 'mgmin']:
        #             if sttg in ['min', 'gmin', 'mmin', 'mgmin']: rsttg = f'{sttg}_{target}'
        #             else: rsttg = sttg
        #             last_idx_tmp = edata[task_name][str(r0_idx)][f'prdts_{rsttg}'].columns[-1]
        #             if int(last_idx_tmp) < last_idx: last_idx = int(last_idx_tmp)
        #         last_idxs[target].append(str(last_idx))
        # print(last_idxs)
        for sttg, target in itertools.product(['zero_vac', 'under_20', '20-49', '20+', '60+', 'all_ages', 'min', 'gmin', 'mmin', 'mgmin'], ['c', 'd', 'y']):
            ...
            for r0_idx in range(120):
                ...
                # print(vac_deadline, int(last_idx), edata[task_name][str(r0_idx)][f'prdts_{rsttg}'].columns[-1], task_name, str(r0_idx), f'prdts_{rsttg}')
