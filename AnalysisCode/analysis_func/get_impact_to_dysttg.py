...
def get_direct_eff(self, daily_vac, prdt_type = 'non_mar'):
    vac_tmp = np.ones(len(self.get_populations())) / self.get_populations() * self._sir_delta__get_eff() * self.get_eta()
    pos = self._sir_delta__time_int
    svu_arr = self._sir_delta__sv[pos] + self._sir_delta__u[pos]
    c_arr = self._sir_delta__c[pos]
    c_prdt_add = self.calc_prdt(prdt_type = prdt_type) - c_arr
...
def analyze(edata):
    ...
    res = {}
    for r0_idx, r0 in enumerate([1.4, 2.5]):
        ...
        for sttg, (prdt_type, target_type) in prdt_target_types.items():
            for target in ['c', 'd', 'y']:
                if target_type =='steady': res_tmp = -target_coef[target][:, None] * calc_non.calc_steady_grad(np.zeros(group_amount)) / calc_params['populations'][None, :]
                elif target_type == 'trans': res_tmp = -target_coef[target][:, None] * (calc_non.calc_trans_grad(prdt_type = prdt_type) * np.eye(group_amount)) / calc_params['populations'][None, :]
                else: pass
                res[f'{sttg}_{target}_{r0_idx}'] = {str(i): res_tmp[:, i] for i in range(len(calc_params['populations']))}
                if target_type == 'steady': 
                    res_tmp = target_coef[target][:, None] * calc_non.get_direct_eff(daily_vac, prdt_type = prdt_type)
                    res[f'dir_{sttg}_{target}_{r0_idx}'] = {str(i): res_tmp[:, i] for i in range(len(calc_params['populations']))}
...
# def get_direct_eff(self, daily_vac, prdt_type = 'non_mar'):
#     vac_tmp = daily_vac * np.ones(len(self.get_populations())) / self.get_populations() * self._sir_delta__get_eff()
#     pos = self._sir_delta__time_int
#     svu_arr = self._sir_delta__sv[pos] + self._sir_delta__u[pos]
#     c_arr = self._sir_delta__c[pos]
#     c_prdt_add = self.calc_prdt(prdt_type = prdt_type) - c_arr
...
#         group_amount = len(calc_params['populations'])
#         vac_allocs = daily_vac * np.eye(group_amount) / calc_params['populations']
#         for sttg, (prdt_type, target_type) in prdt_target_types.items():
#             for target in ['c', 'd', 'y']:
#                 if target_type =='steady': res_tmp = -target_coef[target][None, :] * np.array([calc_non.calc_vac_prdt(vac_allocs[i], prdt_type = prdt_type) - calc_non.calc_prdt(prdt_type = prdt_type) for i in range(group_amount)])
#                 elif target_type == 'trans': res_tmp = -target_coef[target][None, :] * calc_non.calc_trans_grad(prdt_type = prdt_type) * np.array(vac_allocs) / (1 / basic_params.day_div)
#                 else: pass
#                 res[f'{sttg}_{target}_{r0_idx}'] = {str(i): res_tmp[:, i] for i in range(len(calc_params['populations']))}
#                 if target_type == 'steady': 
#                     res_tmp = target_coef[target][None, :] * calc_non.get_direct_eff(daily_vac, prdt_type = prdt_type)
