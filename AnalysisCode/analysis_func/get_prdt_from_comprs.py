...
def analyze(edata):
    ...
    r0_idx = 90
    anal_data = {}
    expr_name = 'optm_from_comprs'
    expr_params = ['lower', 'equal']
    ...
    for expr_param in expr_params:
        ...
        for sttg, target in itertools.product(['zero_vac', 'under_20', '20-49', '20+', '60+', 'all_ages', 'min', 'gmin', 'mmin', 'mgmin'], ['c', 'd', 'y']):
            ...
            for i in range(120):
                last_idx = edata[task_name][str(i)][f'prdts_{rsttg}'].columns[-1]
                anal_data[sheet_name][f'prdt_{rsttg}_{target}'].append(edata[task_name][str(i)][f'prdts_{rsttg}'][str(vac_deadline) if vac_deadline < int(last_idx) else last_idx].to_numpy() @ pop_coef[target])
        
        sheet_name = f'{expr_param}_vac_dur'
        ...
        for sttg, target in itertools.product(['zero_vac', 'under_20', '20-49', '20+', '60+', 'all_ages', 'min', 'gmin', 'mmin', 'mgmin'], ['c', 'd', 'y']):
            ...
            for i in range(1, 121):
                anal_data[sheet_name][f'prdt_{rsttg}_{target}'].append(edata[task_name][str(r0_idx)][f'prdts_{rsttg}'][str(i) if i < int(last_idx) else last_idx].to_numpy() @ pop_coef[target])
