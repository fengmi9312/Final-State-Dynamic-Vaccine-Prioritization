# -*- coding: utf-8 -*-
"""
Created on Sun Jun 22 14:57:00 2025
...
def execute(expr_param, file_idx):
    ...
    # print(unit_beta_inf, unit_beta_rem)
    # print(func.get_mean_from_cum(alpha_inf, unit_beta_inf, alpha_rem, unit_beta_rem) / func.get_mean_from_weibull(alpha_rem, unit_beta_rem))
    ...
    r0 = [1.5, 2.1, 3.5, 1.8, 2.3][file_idx]
    ...
    mar_calc_params['srv_inf'] = func.srv_exponent(inf_res.x[0] * beta_coef, basic_params.srv_length, 1 / basic_params.day_div)
    mar_calc_params['srv_rem'] = func.srv_exponent(rem_res.x[0] * beta_coef, basic_params.srv_length, 1 / basic_params.day_div)
    ...
    for sttg in sttgs:
        ...
        if sttg.split('_')[0] == 'mmin' or sttg.split('_')[0] == 'mgmin':
            ...
            day_idx = 0
            while True:
                if calc_mar.getc_x_tot('s') == 0 or day_idx >= 60:  break
            ...
            day_idx = 0
            while True:
                if calc_non.getc_x_tot('s') == 0 or day_idx >= 60:  break
        ...
        while True:
            calc_non.spread_once()
            if calc_non.get_current_time_int() >= [500, 200, 200, 500, 200][file_idx] * basic_params.day_div: break
        sheet_name = f'curve_{sttg}'
        res[sheet_name] = {}
        res[sheet_name]['time_line'] = calc_non.get_time_line()
        for target in basic_params.all_targets: res[sheet_name][f'curve_{target}'] = calc_non.get_x_tot(target) 
