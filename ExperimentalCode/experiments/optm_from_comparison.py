...
def execute(expr_param, file_idx):
    ...
    inf_pow, rem_pow = {'lower': (3, 15), 'higher': (15, 3), 'equal': (9, 9)}[expr_param]
    alpha_inf = np.e ** (inf_pow * 0.05)
    alpha_rem = np.e ** (rem_pow * 0.05)
    unit_mean_inf, unit_mean_rem = 5, 7
    unit_beta_inf = get_beta_from_weibull(alpha_inf, unit_mean_inf)
    unit_beta_rem = get_beta_from_weibull(alpha_rem, unit_mean_rem)
    ...
    beta_coef = func.find_weibull_coef_from_g(0.14, r0, alpha_inf, alpha_rem, unit_beta_inf, unit_beta_rem, 160000, 1 / basic_params.day_div)
    calc_params['srv_inf'] = func.srv_weibull(alpha_inf, unit_beta_inf * beta_coef, basic_params.srv_length, 1 / basic_params.day_div)
    calc_params['srv_rem'] = func.srv_weibull(alpha_rem, unit_beta_rem * beta_coef, basic_params.srv_length, 1 / basic_params.day_div)
    ...
    mar_calc_params = deepcopy(calc_params)
    mar_calc_params['srv_inf'] = func.srv_exponent(inf_res.x[0], basic_params.srv_length, 1 / basic_params.day_div)
    mar_calc_params['srv_rem'] = func.srv_exponent(rem_res.x[0], basic_params.srv_length, 1 / basic_params.day_div)
    res = {}
    for sttg in sttgs:
        print(sttg)
        if sttg.split('_')[0] == 'mmin' or sttg.split('_')[0] == 'mgmin':
            calc_mar = sir_delta(**mar_calc_params)
            for i in range(vac_onset * basic_params.day_div):
                calc_mar.spread_once()
            res[f'allocs_{sttg}'] = {}
            day_idx = 0
            while True:
                if calc_mar.getc_x_tot('s') == 0:  break
                if sttg.split('_')[0] == 'mmin': alloc = calc_mar.optimize_vac_alloc(daily_vac, target = sttg.split('_')[-1], disp = False)
                elif sttg.split('_')[0] == 'mgmin': alloc = calc_mar.optimize_vac_alloc(daily_vac, target = sttg.split('_')[-1], disp = False, prdt_type = 'grad', trans_delay = 1)
                res[f'allocs_{sttg}'][str(day_idx)] = alloc
                calc_mar.add_vaccination(alloc)
                for vac_idx in range(calc_mar.get_day_div()):
                    calc_mar.spread_once()
                day_idx += 1
            ...
            for day_idx in range(len(res[f'allocs_{sttg}'])):
                res[f'prdts_{sttg}'][str(day_idx)] = calc_non.calc_vac_prdt(res[f'allocs_{sttg}'][str(day_idx)])
                calc_non.add_vaccination(res[f'allocs_{sttg}'][str(day_idx)])
...
# total_res = [execute('equal', i) for i in [27]]
