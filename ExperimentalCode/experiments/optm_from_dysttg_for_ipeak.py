...
def load_saved_allocs(source_expr_name, source_expr_param, file_idx):
    """
    Load allocation sheets generated by a previous experiment.

    Parameters
    ----------
    source_expr_name : str
        Experiment name used to generate the allocation file, e.g. 'optm_from_dysttg'.
    source_expr_param : str
        Task parameter used to generate the allocation file, e.g. 'lower', 'higher', or 'equal'.
    file_idx : int
        File index.
    ...
    dict[str, pd.DataFrame]
        Keys are strategy names, values are allocation DataFrames.
        For example, sheet 'allocs_min_c' becomes key 'min_c'.
    """
    if source_expr_name is None:
        return None

    source_task_name = f'{source_expr_name}_({source_expr_param})'
    source_file_path = name_principle.experimental_file(source_task_name, file_idx)

    if not os.path.exists(source_file_path):
        raise FileNotFoundError(f'Cannot find saved allocation file: {source_file_path}')

    sheets = pd.read_excel(source_file_path, sheet_name=None)

    saved_allocs = {}
    for sheet_name, sheet_data in sheets.items():
        if not sheet_name.startswith('allocs_'):
            continue

        sttg_name = sheet_name.replace('allocs_', '')

        # Remove Excel index column if present
        unnamed_cols = [col for col in sheet_data.columns if str(col).startswith('Unnamed')]
        if unnamed_cols:
            sheet_data = sheet_data.drop(columns=unnamed_cols)

        saved_allocs[sttg_name] = sheet_data.astype(float)

    return saved_allocs


def get_saved_alloc(saved_allocs, sttg, day_idx):
    """
    Get allocation vector for one strategy and one vaccination day.
    """
    if saved_allocs is None:
        return None

    if sttg not in saved_allocs:
        return None

    alloc_df = saved_allocs[sttg]

    # The original code saves daily allocations using str(day_idx) as column name.
    col_name = str(day_idx)
    if col_name not in alloc_df.columns:
        return None

    return alloc_df[col_name].to_numpy(dtype=float)
...
def execute(expr_param, file_idx):
    ...
    saved_allocs = load_saved_allocs('optm_from_dysttg', expr_param, file_idx)
    ...
    for sttg in sttgs:
        ...
        while True:
            ...
            alloc = get_saved_alloc(saved_allocs, sttg, day_idx)
            if alloc is None:
                if day_idx > 0: break
        ...
        while True:
            calc_non.spread_once()
            if calc_non.getc_x_tot('i') < 1e-6:
                break
        res[f'ipeak_{sttg}'] = {'res': calc_non.get_x_tot('i')}
