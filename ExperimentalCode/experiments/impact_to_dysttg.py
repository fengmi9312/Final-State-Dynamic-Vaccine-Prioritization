...
def execute(expr_param, file_idx):
    ...
    #d_prdt = calc_params['ifrs'] * np.array([calc_non.calc_vac_prdt(np.ones(group_amount)[i] / calc_params['populations'][i]) - calc_non.calc_prdt() for i in range(group_amount)])
    for sttg, (prdt_type, target_type) in prdt_target_types.items():
        if target_type =='steady': res_tmp = - (calc_params['populations'] * calc_params['ifrs'])[None, :] * np.array([calc_non.calc_vac_prdt(vac_allocs[i], prdt_type = prdt_type) - calc_non.calc_prdt(prdt_type = prdt_type) for i in range(group_amount)])
        elif target_type == 'trans': res_tmp = - (calc_params['populations'] * calc_params['ifrs'])[None, :] * calc_non.calc_trans_grad(prdt_type = prdt_type) * np.array(vac_allocs)
        else: pass
        res[sttg] = {str(i): res_tmp[:, i] for i in range(len(calc_params['populations']))}
    return {sheet_name: pd.DataFrame(sheet_data) for sheet_name, sheet_data in res.items()} 



if __name__ == '__main__':
    total_res = [execute('param', i) for i in [0]]
