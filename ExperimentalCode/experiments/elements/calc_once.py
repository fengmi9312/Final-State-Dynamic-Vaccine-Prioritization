...
def simulate(calc_params, r0 = 2, vac_eff = 0.95, daily_vac = 1e-2, vac_start = 30, vac_dur = None, 
             ...
             get_curve = False, get_effr = False, get_empirical = True, get_trans = False, get_grad = False, optm_targets = ['c', 'd', 'y'], prdt_targets = ['c', 'd', 'y'], optm_disp = False, tols = {'c': 1e-16, 'd': 1e-16, 'y': 1e-16}):
    if k_val is None:
    ...
    if get_grad: sttgs += [f'gmin_{optm_target}' for optm_target in optm_targets]
    for sttg in sttgs:
        ...
        if sttg != 'no_vac':
            ...
            while True:
                ...
                if sttg.split('_')[0] == 'min': alloc = calc_once.optimize_vac_alloc(daily_vac, target = sttg.split('_')[-1], disp = optm_disp, tol = tols[sttg.split('_')[-1]])
                ...
                elif sttg.split('_')[0] == 'gmin': alloc = calc_once.optimize_vac_alloc(daily_vac, target = sttg.split('_')[-1], disp = optm_disp, prdt_type = 'grad', trans_delay = 1)
